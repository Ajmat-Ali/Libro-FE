...
const UpdateMemberDrawer = ({ member, onClose, onSuccess }) => {
  ...
  const onSubmit = async (data) => {
    try {
      setServerError("");
      await updateMember(member?.profile?.userId?._id, {
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full sm:w-[440px] bg-slate-50 dark:bg-slate-900 z-50 flex flex-col shadow-2xl border-l border-slate-200 dark:border-slate-700 animate-[slideInRight_0.3s_ease-out]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {serverError && (
            <div className="p-3 mb-4 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-xl text-red-600 dark:text-red-400 text-sm">
              {serverError}
            </div>
          )}
          ...
          >
            ...
            <div>
              ...
              <textarea
                ...
                className={`${inputClass("address", errors.address)} resize-none`}
              />
              {errors.address && (
                <p className="text-red-500 text-xs mt-1">
            ...
            <div>
              ...
              <input
                ...
                value={user?.email || ""}
                disabled
                className="w-full px-3.5 py-2.5 rounded-xl border text-sm text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 cursor-not-allowed"
              />
            </div>
          </form>
        </div>
        ...
        <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-2.5 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-200 dark:disabled:bg-slate-700 disabled:text-slate-400 dark:disabled:text-slate-500 disabled:cursor-not-allowed text-slate-900 font-semibold rounded-xl transition-all text-sm flex items-center justify-center gap-2"
          >
            {isSubmitting ? (
              <>
                <svg
