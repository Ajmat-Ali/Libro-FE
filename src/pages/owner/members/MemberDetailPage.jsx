...
const InfoRow = ({ icon: Icon, label, value }) => {
  ...
  return (
    <div className="flex items-start gap-3 py-3 border-b border-slate-100 dark:border-slate-700/50 last:border-0">
      <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-700/50 flex items-center justify-center flex-shrink-0 mt-0.5">
...
const MemberDetailPage = () => {
  ...
  if (error || !member) {
    return (
      <div className="p-6">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-900 dark:hover:text-white mb-4"
        >
  ...
  const user = member?.profile?.userId || member;
  const isActive = user?.isActive ?? true;
  const statusKey = !isActive
    ? "suspended"
    : member?.profile?.approvalStatus || "pending";
        ...
        <div className="bg-slate-900 px-4 lg:px-6 pt-6 pb-16">
          ...
          <div className="flex items-start gap-5">
            <div className="relative flex-shrink-0">
              <div
                className={`w-16 h-16 rounded-full bg-amber-500 flex items-center justify-center text-slate-900 text-xl font-bold ring-2 ring-offset-2 ring-offset-slate-900 ${status.ring}`}
              >
              ...
              <span
                className={`absolute bottom-0.5 right-0.5 w-3.5 h-3.5 rounded-full border-2 border-slate-900 ${status.dot}`}
              />
            </div>
            ...
            <div className="flex-1 min-w-0 ">
              <div className="flex items-start gap-3 flex-wrap">
                ...
                <span
                  className={`text-xs px-2.5 py-1 rounded-full font-semibold flex-shrink-0 ${status.badge}`}
                >
                  {status.label}
                </span>
              </div>
              ...
              {member?.profile?.membershipId && (
                <p className="text-amber-400 text-xs font-mono mt-1.5 tracking-wide">
                  {member?.profile?.membershipId}
                </p>
              )}
            </div>
            ...
            <button
              ...
              className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 text-white text-sm font-medium rounded-xl transition-all"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-100 dark:border-slate-700">
                ...
                <InfoRow
                  ...
                  value={
                    member?.profile?.isEmailVerified
                      ? "Verified ✓"
                      : "Not verified"
                  }
                />
              ...
              <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-100 dark:border-slate-700">
                ...
                <InfoRow
                  icon={Shield}
                  label="Membership ID"
                  value={member?.profile?.membershipId || "Not assigned yet"}
                />
                {member?.profile?.reviewedAt && (
                  <InfoRow
                    icon={Calendar}
                    label="Reviewed On"
                    value={new Date(
                ...
                {member?.profile?.rejectionReason && (
                  <div className="mt-2 p-3 bg-red-50 dark:bg-red-500/10 rounded-xl border border-red-100 dark:border-red-500/20">
            ...
            <div className="space-y-4">
              <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-100 dark:border-slate-700">
                ...
                <button
                  onClick={() => setShowUpdate(true)}
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm font-medium transition-all mb-2"
                >
              ...
              <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-red-100 dark:border-red-500/20">
